...
object TrackerRegistry {
  ...
  private[borg] class TrackerRegistryActor(config: ZooKeeperClientConfig, testProbe: Option[ActorRef] = None) extends Actor with Logging with Listeners {
    ...
    private def childrenChanged(children: Seq[String]) {
      ...
      addedChildren foreach { child ⇒ zk.watchNode(node(child), nodeChanged(child)) }
